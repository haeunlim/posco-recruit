  ...
  var modalFn = {
    show: function (t, params) {
      ...
      if (typeof t != "object" && $(params.bxId).length === 0) {
        var bx_id = params.bxId.substring(
          params.bxId.indexOf("#") + 1,
          params.bxId.indexOf(".") === -1
            ? params.bxId.length
            : params.bxId.indexOf(".")
        );
        ...
        $("body").append(
          $("<section></section>").prop({ id: bx_id }).addClass(bx_class)
        );
      }
      if ($(params.bgId).length === 0) {
        var bg_id = params.bgId.substring(
          params.bgId.indexOf("#") + 1,
          params.bgId.indexOf(".") === -1
            ? params.bgId.length
            : params.bgId.indexOf(".")
        );
      ...
      function show() {
        ...
        t.imagesLoaded(function () {
          if ($(".modal.on").length > 0)
            params.parent = $("#" + $(".modal.on").attr("id"));
          bg.addClass("on");
          if (params.bgClose) {
    ...
    resize: function (e) {
      ...
      var bxHeadH =
        t.find(".modal_header").length != 0
          ? t.find(".modal_header").outerHeight()
          : 0;
      var bxFootH =
        t.find(".modal_footer").length != 0
          ? t.find(".modal_footer").outerHeight()
          : 0;
      var bxCont = t.find(".modal_content");
      var scl = posi == "fixed" ? 0 : $(window).scrollTop();
      bxCont.css({ height: "" });
      var bxH = t.outerHeight();
      if (bxCont.outerHeight() > bxH - bxHeadH - bxFootH)
        bxCont.css({ height: bxH - bxHeadH - bxFootH + 1 });
      bxH = t.outerHeight();
